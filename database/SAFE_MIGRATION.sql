-- ============================================
-- SmartSpace - SAFE MIGRATION (Preserves Existing Data)
-- ============================================
-- ✅ This will NOT delete your existing data!
-- It only ADDS new columns and tables
-- ============================================

-- Step 1: Add missing columns to existing warehouses table
DO $$ 
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Set all existing warehouses to active
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name='warehouses' AND column_name='district'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name='warehouses' AND column_name='total_blocks'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.profiles ADD COLUMN seeker_type TEXT 
...
-- Step 3: Add missing column to existing bookings table
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name='bookings' AND column_name='product_details'
  ) THEN
    ALTER TABLE public.bookings ADD COLUMN product_details JSONB;
  END IF;
END $$;

-- Step 4: Create NEW tables only (won't affect existing data)
...
-- Step 8: Update existing warehouse policies (drop and recreate with new columns)
...
-- Step 9: Insert Maharashtra pricing data (only if table is empty)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.pricing_reference LIMIT 1) THEN
...
-- Step 10: Insert product pricing data (only if table is empty)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.product_market_prices LIMIT 1) THEN
...
-- Step 11: Insert storage configurations (only if table is empty)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.storage_rate_config LIMIT 1) THEN
...
SELECT 
  '✅ Migration complete! Existing data preserved.' as status,
  (SELECT COUNT(*) FROM warehouses) as existing_warehouses,
